...
func CreateBlockChainWithGenesisBlock(data string) *BlockChainQB {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			err = b.Put(genesisBlock.Hash, genesisBlock.SerializeQB())
			if err != nil {
				log.Panic(err)
			}
	...
	return &BlockChainQB{db, genesisBlock.Hash}
}
...
func (QB *BlockChainQB) AddBlockToBlockChainQB(data string) {
	...
	err := QB.DB.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			//序列化后存入到数据库中
			err := b.Put(newBlock.Hash, newBlock.SerializeQB())
			if err != nil {
				log.Panic(err)
			}
