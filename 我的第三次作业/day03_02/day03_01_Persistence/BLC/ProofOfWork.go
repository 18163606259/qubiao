...
func NewProofOfWorkQB(block *Block) *ProofOfWork {
	//1.创建pow对象
	pow := &ProofOfWork{}
	//2.设置属性值
	pow.Block = block
...
func (pow *ProofOfWork) prepareDataQB(nonce int64) []byte {
	//1.根据nonce，生成pow中要验证的block的数组
	data := bytes.Join([][]byte{
		IntToHex(pow.Block.HeightQB),
		pow.Block.PrevBlockHashQB,
		IntToHex(pow.Block.TimeStampQB),
		pow.Block.DataQB,
		IntToHex(nonce),
		IntToHex(TargetBit),
	}, []byte{})
...
func (pow *ProofOfWork) IsValid()bool{
	hashInt :=new(big.Int)
	hashInt.SetBytes(pow.Block.HashQB)
